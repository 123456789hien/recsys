...
function parseUsers(text){
    ...
    users = lines.filter(l=>l.trim()!=="").map(l=>{
        const [user_id, location] = l.trim().split(delimiter);
...
function parseItems(text){
    ...
    items = lines.filter(l=>l.trim()!=="").map(l=>{
        const [item_id, name, price] = l.trim().split(delimiter);
...
function parseInteractions(text){
    ...
    interactions = lines.filter(l=>l.trim()!=="").map(l=>{
        const [user_id, item_id, timestamp, event_type, rating] = l.trim().split(delimiter);
