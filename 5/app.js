d3.csv("fashion_recommender.csv").then(data => {
    ...
    const insights = [
        ...
        "Most popular category: " + getTopCategories(data),
        "High rating products can be promoted for trust-building.",
        "Price/Rating ratio helps prioritize recommendations."
    ];
...
// --- Helper Functions ---
function getUnique(data, key){ return Array.from(new Set(data.map(d=>d[key]))).sort(); }
function populateFilter(id, values){ const select = document.getElementById(id); values.forEach(v=>{ const opt=document.createElement("option"); opt.value=v; opt.textContent=v; select.appendChild(opt); }); }
function getTopCategories(data){ const counts={}; data.forEach(d=>counts[d.Category]=(counts[d.Category]||0)+1); return Object.entries(counts).sort((a,b)=>b[1]-a[1])[0][0]; }
...
function renderTopRecommendations(data){
    const top5=data.sort((a,b)=>{ if(b.Rating===a.Rating) return a.Price-b.Price; return b.Rating-a.Rating; }).slice(0,5);
