/* app.js
   ...
   - Fixed category matching to avoid "men" substring matching inside "women"
*/

/* ============== CONFIG ==============
 * If you want auto-load from GitHub raw, set dataUrl to that raw CSV URL.
 * Example: https://raw.githubusercontent.com/username/repo/branch/path/fashion_recommender.csv
 */
const dataUrl = "https://raw.githubusercontent.com/123456789hien/recsys/refs/heads/main/hd_midtermexam/fashion_recommender.csv";
// Provide AUTOLOAD_URL for backward compatibility in UI click check
...
function toNumber(value) {
  ...
  if (s.indexOf(",") !== -1 && s.indexOf(".") !== -1) {
    // if both exist, assume commas are thousands separators
    s = s.replace(/,/g, "");
...
function normalizeCategoryString(s) {
  if (!s && s !== 0) return "";
  // lowercase, replace non-alphanum by spaces, trim
...
function handleRawCSV(text) {
  ...
  data = rows.map(r => {
    ...
    // normalize category values to match buttons if possible (robust)
...
function getFilteredData() {
  ...
  // 1) Prefer exact match (we normalized categories when loading)
  ...
  // 3) Token-based matching fallback (match all meaningful tokens, ignore single-letter tokens like 's')
  ...
  return data.filter(d => {
    ...
    // ensure every token in target is present in dTokens (prevents "men" matching inside "women")
...
function renderTop10Tables(filtered) {
  ...
  const scored = filtered.map(p => {
    ...
    const score = 0.5*nr + 0.3*np + 0.2*Math.random(); // keep random_diversity per original logic
    return {...p, score};
