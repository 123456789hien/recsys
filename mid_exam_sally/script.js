...
document.getElementById("recommend-btn").onclick = function() {
    ...
    interactions.forEach(i=>{
        if(!itemRatings[i.item_id]) itemRatings[i.item_id] = [];
        itemRatings[i.item_id].push(i.rating);
    });
    const avgRatings = {};
    Object.keys(itemRatings).forEach(id=>{
        const arr = itemRatings[id];
        avgRatings[id] = arr.reduce((a,b)=>a+b,0)/arr.length;
    });

    // Compute top-N
    const topN = items.map(it=>{
        const ratingScore = avgRatings[it.item_id] || 0;
        const s = (simScores[it.item_id]||0) + ratingScore;
        const score = it.price>0 ? s/it.price : s;
        return {...it, score, rating: ratingScore};
