...
// Build optimized Two-Tower model
...
// Train model client-side (optimized)
async function trainTwoTower(dataset){
    ...
    await model.fit([dataset.xs_user, dataset.xs_item], dataset.ys,{
        batchSize:128, epochs:3, verbose:0
    });
...
document.getElementById("recommend-btn").onclick = async function() {
    ...
    dataset.itemIds.forEach((it,i)=>{
        tf.tidy(()=>{
            ...
            scores.push({...item, score, similarity: pred.dataSync()[0]});
        });
    });

    // Top-10 categories
    const bestValue = scores.sort((a,b)=>b.score-a.score).slice(0,10);
    const highestRated = items.map(it=>{
        const r = interactions.filter(i=>i.item_id===it.item_id).reduce((a,b)=>a+b.rating,0);
        return {...it, avgRating: r};
    }).sort((a,b)=>b.avgRating-a.avgRating).slice(0,10);
    const mostSimilar = scores.sort((a,b)=>b.similarity-a.similarity).slice(0,10);

    populateRecTable("best-value-table", bestValue, true);
    populateRecTable("highest-rated-table", highestRated, false, true);
    populateRecTable("most-similar-table", mostSimilar, false, false, true);
...
// Populate table
function populateRecTable(tableId, data, includeScore=false, includeRating=false, includeSimilarity=false){
    ...
    data.forEach((item,idx)=>{
        ...
        tr.innerHTML = `<td>${idx+1}</td>
                        <td>${item.name}</td>
                        ${includeRating?`<td>${item.avgRating.toFixed(1)}</td>`:''}
                        <td>$${item.price.toFixed(2)}</td>
                        ${includeScore?`<td>${item.score.toFixed(2)}</td>`:''}
                        ${includeSimilarity?`<td>${item.similarity.toFixed(2)}</td>`:''}`;
