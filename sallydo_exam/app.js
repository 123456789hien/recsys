/* app.js
   ...
   - Fixed: AUTOLOAD_URL, robust category filtering, added Category column in top tables
*/
 ...
 */
const dataUrl = "https://raw.githubusercontent.com/123456789hien/recsys/refs/heads/main/sallydo_exam/fashion_recommender.csv";
...
/* filter helper (robust for different naming in CSV) */
function getFilteredData() {
  ...
  return data.filter(d => {
    const dcat = normalizeCategoryString(d.category || "");
    // accept if exact normalized match or if one includes the other
    return dcat === targetNorm || dcat.includes(targetNorm) || targetNorm.includes(dcat);
  });
}
...
function getBuckets(values, nBuckets=10){
  ...
  for (let i=0;i<nBuckets;i++){
    const a = min + i*step;
    const b = a + step;
    labels.push(`${a.toFixed(2)} - ${b.toFixed(02)}`);
  }
  values.forEach(v=>{
